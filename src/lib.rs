...
#[pyfunction]
fn rust_par_mc_pricer(df: PyDataFrame, rate: f64, n_sims: i32) -> PyResult<PyDataFrame> {
    let rdf =
        rust_pricer::rust_par_mc_pricer(df.into(), rate, n_sims).map_err(PyPolarsErr::from)?;
    Ok(PyDataFrame(rdf))
}
