...
pub fn rust_par_mc_pricer(mut df: DataFrame, rate: f64, n_sims: i32) -> PolarsResult<DataFrame> {
    let n_assets = df.height();
    let sigma: Vec<f64> = df
        .column("sigma")?
        .f64()?
