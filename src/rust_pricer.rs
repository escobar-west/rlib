...
pub fn mc_pricer(df: DataFrame, rate: f64, n_sims: i32) -> PolarsResult<DataFrame> {
    ...
    let mut avg_price = Vec::with_capacity(n_assets).resize(n_assets, 0.0f64);
    let df = df.lazy();
    for _ in 0..n_sims {
        let z: Series = (&mut rng)
            ...
            .take(n_assets)
            .collect();
        let rand_walk = col("sigma") * col("maturity") * z.lit();
        ...
        let output = df.select(&[payoff.alias("payoff")]).collect();
